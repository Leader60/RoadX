...
export function PlaylistsView({ onOpenTrack }: { onOpenTrack: (id: string) => void }) {
  ...
  if (openId === "arabic-gs") {
    return (
      <ArabicPlaylistDetail onBack={() => setOpenId(null)} />
    );
  }
  ...
  return (
    <div className="rx-fade-in flex flex-col gap-4 px-4 py-5 pb-6">
      ...
      {/* قائمة العربي - بطاقة كبيرة */}
      <button
        onClick={() => setOpenId("arabic-gs")}
        className="rx-press flex flex-col overflow-hidden rounded-2xl border border-gold/30 bg-card transition-colors hover:border-gold"
      >
        <div className="relative h-40 w-full overflow-hidden">
          <img
            src={imageUrl(ARABIC_PLAYLIST.query, 640, 360) || "/placeholder.svg"}
            alt={ARABIC_PLAYLIST.title}
  ...
  return (
    ...
    >
      <div className="relative aspect-square w-full overflow-hidden">
        <img
          src={`https://e-cdns-images.dzcdn.net/images/playlist/${playlist.id}/500x500-000000-80-0-0.jpg`}
          alt={playlist.title}
          className="h-full w-full object-cover"
          onError={(e) => {
            (e.target as HTMLImageElement).src = imageUrl(playlist.query, 320, 320);
          }}
        />
...
// ========================
function ArabicPlaylistDetail({ onBack }: { onBack: () => void }) {
  const [tracks, setTracks] = useState<ArabicTrack[]>([]);
  ...
  const fetchArabicTracks = async () => {
    setLoading(true);
    setError("");
    try {
      const res = await fetch(ARABIC_PLAYLIST_SHEET);
      const csv = await res.text();
      const lines = csv.trim().split("\n");
      if (lines.length < 2) { setTracks([]); setLoading(false); return; }

      const headers = lines[0].split(",").map((h: string) => h.trim());
      const data: ArabicTrack[] = lines.slice(1).map((line: string) => {
        const values = line.split(",").map((v: string) => v.trim().replace(/^"|"$/g, ""));
        const obj: Record<string, string> = {};
        headers.forEach((h: string, i: number) => { obj[h] = values[i] || ""; });
        return {
          id: obj.id || "",
          title: obj.title || "غير معروف",
          artist: obj.artist || "غير معروف",
          image: obj.image || "",
          youtube: obj.youtube || "",
        };
      });
    ...
    <div className="rx-fade-in flex flex-col gap-4 pb-6">
      <div className="relative">
        ...
        <div className="absolute bottom-3 right-4 left-4">
          <h1 className="text-2xl font-bold text-foreground">{ARABIC_PLAYLIST.title}</h1>
        </div>
      </div>
      ...
      {error && (
        <div className="text-center py-12 text-red-400">
          ...
          <button onClick={fetchArabicTracks} className="mt-3 px-5 py-2 bg-gold text-gold-foreground rounded-xl rx-press text-sm">إعادة المحاولة</button>
        </div>
      )}
      ...
      {!loading && !error && tracks.length === 0 && (
        <div className="text-center py-12 text-muted-foreground text-sm"><p>لا توجد أغاني</p></div>
      )}
    </div>
  );
}
  ...
  return (
    <div className="rx-fade-in flex flex-col gap-4 pb-6">
      <div className="relative">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-background via-background/40 to-navy-deep/30" />
        <button onClick={onBack} aria-label="رجوع" className="rx-press absolute right-3 top-3 flex h-10 w-10 items-center justify-center rounded-full bg-navy-deep/70 text-gold">
          <IconChevronLeft size={22} />
        </button>
        <div className="absolute bottom-3 right-4 left-4">
